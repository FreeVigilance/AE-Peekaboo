...
async def collect_prometheus_metrics(
    ...
    """
    Middleware для сбора метрик Prometheus по HTTP-запросам.
...
def _get_route_name(
    ...
    """
    Рекурсивно находит имя маршрута, соответствующего переданному объекту `scope`.

    Args:
        scope: Словарь, содержащий информацию о запросе, включая путь и другие параметры.
        routes: Список маршрутов, определенных в приложении FastAPI.
        route_name: Временное имя маршрута, которое передается рекурсивно. Defaults to None.
